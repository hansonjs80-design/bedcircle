  ...
  return (
    <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar min-h-[60px]">
          {preset.steps.length === 0 ? (
            ...
            preset.steps.map((step, idx) => (
              <div key={idx} className="flex items-center gap-2 p-1.5 bg-gray-50 dark:bg-slate-700/30 border border-gray-100 dark:border-slate-700 rounded-xl shadow-sm transition-all hover:bg-white dark:hover:bg-slate-700/50">
                
                {/* 순서 변경 컨트롤 (좌측) */}
                <div className="flex flex-col bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-lg overflow-hidden shadow-sm shrink-0">
                  <button 
                    onClick={() => moveStep(idx, 'up')} 
                    disabled={idx === 0} 
                    className="p-1 text-gray-400 disabled:opacity-10 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors active:scale-90"
                    aria-label="위로 이동"
                  >
                ...
                {/* 색상 표시 바 (h-8로 높이 축소) */}
                <div className={`w-1 h-8 rounded-full shrink-0 ${step.color}`} />
                ...
                {/* 시간 조정기 (우측) - 수평 배치 및 버튼 사이즈 축소 */}
                <div className="flex items-center bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg p-0.5 shadow-inner shrink-0 gap-1">
                  <div className="flex items-center">
                    <button 
                      onClick={() => updateDuration(idx, -1)} 
                      className="w-6 h-6 flex items-center justify-center rounded-md hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-500 transition-colors active:scale-90"
                    >
                      <Minus className="w-3 h-3" strokeWidth={3} />
                    </button>
                    
                    <div className="px-1 min-w-[36px] flex items-center justify-center gap-0.5 leading-none">
                      <span className="text-sm font-black text-brand-600 dark:text-brand-400">
                        {Math.floor(step.duration / 60)}
                      </span>
                      <span className="text-sm font-black text-brand-600 dark:text-brand-400">분</span>
                    </div>

                    <button 
                      onClick={() => updateDuration(idx, 1)} 
                      className="w-6 h-6 flex items-center justify-center rounded-md hover:bg-gray-100 dark:hover:bg-slate-700 text-brand-600 transition-colors active:scale-90"
                    >
                  ...
                  {/* 삭제 버튼 추가 */}
                  <div className="w-px h-3 bg-gray-100 dark:bg-slate-700 mx-0.5" />
                  <button 
